...
<div id="has-many-{{$column}}" class="nav-tabs-custom has-many-{{$column}}">
    <div class="row header">
        ...
        <div class="col-md-8 {{$viewClass['field']}}">
            <button type="button" class="btn btn-default btn-{{ config('admin.form-style') }} add">
    ...
    <template class="nav-tab-tpl">
        <li class="new">
            <a href="#{{ $relationName . '_new_' . \Brazzer\Admin\Form\NestedForm::DEFAULT_KEY_NAME }}"
               data-toggle="tab">
                &nbsp;New {{ \Brazzer\Admin\Form\NestedForm::DEFAULT_KEY_NAME }} <i
    ...
    <template class="pane-tpl">
        <div class="tab-pane fields-group new"
             id="{{ $relationName . '_new_' . \Brazzer\Admin\Form\NestedForm::DEFAULT_KEY_NAME }}">
            {!! $template !!}
        </div>
    </template>
</div>

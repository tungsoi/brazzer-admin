    ...
    <div class="{{$viewClass['field']}}">
        <div id="has-many-{{$column}}" style="margin-top: 15px;">
                ...
                @foreach($forms as $pk => $form)
                    <tr class="has-many-{{$column}}-form fields-group">
                        ...
                        @foreach($form->fields() as $field)
                            @if (is_a($field, \Brazzer\Admin\Form\Field\Hidden::class))
                        ...
                        @if($options['allowDelete'])
                            <td class="form-group">
                                <div>
                                    <div class="remove btn btn-warning btn-{{ config('admin.form-style') }} pull-right">
            ...
            <template class="{{$column}}-tpl">
                <tr class="has-many-{{$column}}-form fields-group">
                    {!! $template !!}
                    <td class="form-group form-group-{{ config('admin.form-style') }}">
                        <div>
                            <div class="remove btn btn-warning btn-{{ config('admin.form-style') }} pull-right">
                                <i class="fa fa-trash">&nbsp;</i>{{ trans('admin.remove') }}
                            </div>

            ...
            <div class="select-options">
                <ul>
                    @foreach($selector['options'] as $value => $option)
                        ...
                        @endphp
                        <li>
                            <a href="{{ \Brazzer\Admin\Grid\Tools\Selector::url($column, $value, true) }}"
                               class="{{$active ? 'active' : ''}}" noloading>{{ $option }}</a>
                            @if(!$active && $selector['type'] == 'many')
                                &nbsp;
                                <a href="{{ \Brazzer\Admin\Grid\Tools\Selector::url($column, $value) }}" class="add" noloading><i
                                            class="fa fa-plus-square-o"></i></a>
                            @else
                                <a style="visibility: hidden;" noloading><i class="fa fa-plus-square-o"></i></a>
                            @endif
                        </li>
                    @endforeach
                    <li>
                        <a href="{{ \Brazzer\Admin\Grid\Tools\Selector::url($column) }}" class="clear"><i

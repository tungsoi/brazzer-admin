    ...
    <div class="col-md-10">
        <div class="box box-primary">
            <div class="box-header with-border">
                <button type="button" class="btn btn-primary btn-xs log-refresh"><i
                            class="fa fa-refresh"></i> {{ trans('admin.refresh') }}</button>
                <button type="button" class="btn btn-default btn-xs log-live"><i class="fa fa-play"></i></button>
                <div class="pull-right">
                    <div class="btn-group">
                        @if ($prevUrl)
                            <a href="{{ $prevUrl }}" class="btn btn-default btn-xs"><i
                                        class="fa fa-chevron-left"></i></a>
                        @endif
                        @if ($nextUrl)
                            <a href="{{ $nextUrl }}" class="btn btn-default btn-xs"><i class="fa fa-chevron-right"></i></a>
                        @endif
                    </div>
                    <!-- /.btn-group -->
                </div>
                ...
                <div class="table-responsive">
                    <table class="table table-hover">
                        ...
                        @foreach($logs as $index => $log)
                            ...
                            <tr>
                                <td>
                                    <span class="label bg-{{\Brazzer\Admin\Extension\LogViewer::$levelColors[$log['level']]}}">{{ $log['level'] }}</span>
                                </td>
                                ...
                                <td>
                                    @if(!empty($log['trace']))
                                        <a class="btn btn-primary btn-xs" data-toggle="collapse"
                                           data-target=".trace-{{$index}}"><i class="fa fa-info"></i>&nbsp;&nbsp;Exception</a>
                                    @endif
                                </td>
                            </tr>
                ...
                <ul class="nav nav-pills nav-stacked">
                    @foreach($logFiles as $logFile)
                        <li @if($logFile == $fileName)class="active"@endif>
                            <a href="{{ route('admin.logviewer.file', ['file' => $logFile]) }}">

...
abstract class RemoteModel implements ArrayAccess, Arrayable, Jsonable, JsonSerializable{
    ...
    public function paginateHydrate($result, $modelClass = null){
        ...
        return new LengthAwarePaginator($this->hydrate($items, $modelClass), $total, $perPage, $page, array_merge($options, [
    ...
    public static function find($id, array $params = []){
        ...
        return $instance->request($instance->getEndpoint(), 'find', [
            $id,
            $params
        ]);
    }
    public static function findWhere($params, $toArray = true){
        try{
            $results = \Socialite::driver('brazzer')->getData('api/' . static::$route . '/all?' . http_build_query($params));
            if(!is_array($results)){
                $results = json_decode($results, true);
            }
            if($toArray){
                $model = new static([]);
                $model->setRawAttributes((array)$results, true);
                return $model;
            }else{
    ...
    public static function findOrFail($id, array $params = []){
        ...
        // Make request
        if(!is_null($result = $instance->request($instance->getEndpoint(), 'find', [
            $id,
            $params
        ]))){
            return $result;
    ...
    public function paginateChildren($modelClass, $method, array $params = []){
        ...
        // Make request
        $result = $this->makeRequest($this->getEndpoint(), $method, [
            $this->getKey(),
            $params
        ]);
    ...
    protected function setKeysForSave(){
        ...
        $params = array_map(function($value){
            if(is_object($value) && !($value instanceof UploadedFile) && is_callable([
                    $value,
                    'toApi'
                ])){
                return $value->toApi();
        ...
        return array_except($params, array_merge([
            static::CREATED_AT,
            static::UPDATED_AT
        ], [
            'id',
    ...
    public function get($id, array $params = []){
        return $this->request($this->getEndpoint(), 'find', [
            $id,
            $params
        ]);
    }
    ...
    protected function isJsonCastable($key){
        if($this->hasCast($key)){
            return in_array($this->getCastType($key), [
                'array',
                'json',
                'object'
            ], true);
    ...
    protected function castAttribute($key, $value){
        ...
        switch($this->getCastType($key)){
            ...
            case 'object':
                return is_array($value) ? json_decode(json_encode($value)) : json_decode($value);
            case 'array':
            case 'json':
    ...
    public function hasMany($related, $parentKey = null, $localKey = null){
        ...
        return new \Gruz\RemoteModel\Relations\Relation($instance, $this, $parentKey, $localKey);
    }
    ...
    public function hasOne($related, $parentKey = null, $localKey = null){
        ...
        $relation = new \Gruz\RemoteModel\Relations\Relation($instance, $this, $parentKey, $localKey);
     ...
     * @return mixed
     */
    protected function makeRequest($endpoint = null, $method = 'get', $params){
        ...
        */
        $results = $this->messageBag = [];
        $methodData = $method . 'Data';
        try{
            $results = \Socialite::driver('brazzer')->$methodData('api/' . $endpoint, $params);
        }catch(\Exception $e){
            \Log::error($e);
            $this->messageBag = $e;
        }
    ...
    protected function hasToArray($value){
        return (is_object($value) && !($value instanceof UploadedFile) && is_callable([
                $value,
                'toArray'
            ]));
    }

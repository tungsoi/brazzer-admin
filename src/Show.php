...
use Brazzer\Admin\Show\Divider;
use Brazzer\Admin\Show\Field;
use Brazzer\Admin\Show\Html;
use Brazzer\Admin\Show\Panel;
use Brazzer\Admin\Show\Relation;
...
class Show implements Renderable{
    ...
    protected function overwriteExistingRelation($name){
        ...
        $this->relations = $this->relations->filter(function(Relation $relation) use ($name){
    ...
    protected function handleRelationField($method, $arguments){
        ...
        if($relation instanceof HasMany || $relation instanceof MorphMany || $relation instanceof BelongsToMany || $relation instanceof HasManyThrough){
    ...
    public function render(){
        ...
        $this->fields->each->setWidth($this->width['field'],$this->width['label']);
    ...
    //hongnn
    public function html($label='', $html = '')
    {
        $this->fields->push(new Html($label, $html));
    }
}

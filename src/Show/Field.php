...
use Brazzer\Admin\Widgets\Carousel;
                ...
                foreach($path as $item){
                    ...
                    if(url()->isValidUrl($item)){
                        ...
                        if($storage->exists($item)){
                            ...
                            $size = ($storage->size($item) / 1000) . 'KB';
                        }
                    }
                    if($url != ''){
                        $html .= '<li style="margin-bottom: 0;">
...
HTML;
    ...
    public function render(){
        if($this->showAs->isNotEmpty()){
            $this->showAs->each(function($callable){
                $this->value = $callable->call($this->parent->getModel(), $this->value);
            });
        }
    ...
    //hongnn
    public function gallery($server = '', $width = 50, $height = 50)
    {
        return $this->unescape()->as(function ($images) use ($server, $width, $height) {
            return collect($images)->map(function ($path) use ($server, $width, $height) {
                if (empty($path)) {
                ...
                if ($ext == 'pdf') {
                    Admin::script("$('.grid-popup-iframe').magnificPopup({
                                            disableOn: 700,
                                            type: 'iframe',
                                            mainClass: 'mfp-fade',
                                            removalDelay: 160,
...
HTML;
                    ...
                    Admin::script('$(\'.grid-popup-link\').magnificPopup({"type":"image","gallery":{"enabled":true,"preload":[0,2],"navigateByImgClick":true,"arrowMarkup":"<button title=\"%title%\" type=\"button\" class=\"mfp-arrow mfp-arrow-%dir%\"><\/button>","tPrev":"Previous (Left arrow key)","tNext":"Next (Right arrow key)","tCounter":"<span class=\"mfp-counter\">%curr% of %total%<\/span>"}});');
                    return <<<HTML
<a href="$src" class="grid-popup-link" noloading>
    <img src='$src' style='max-width:{$width}px;max-height:{$height}px' class='img' />
</a>
HTML;
                }
                return "<img src='$src' style='max-width:{$width}px;max-height:{$height}px' class='img' />";

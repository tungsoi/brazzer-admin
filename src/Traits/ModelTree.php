...
trait ModelTree{
    ...
    public function allNodes(){
        ...
        $conditions = $this->list_query;
        return $self->where(function($query) use ($conditions){
            if($conditions){
                foreach($conditions as $condition){
                    if($condition){
                        foreach($condition as $key => $value){
                            if($value && isset($value[0]) && in_array($value[0], static::getFillable())){
                                if(count($value) == 2){
    ...
    public function list_query($list_query){

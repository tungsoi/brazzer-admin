...
namespace Brazzer\Admin\Tree;

use Brazzer\Admin\Tree\Model;
use Brazzer\Admin\Tree\Filter\AbstractFilter;
use Brazzer\Admin\Tree\Filter\Between;
use Brazzer\Admin\Tree\Filter\Date;
use Brazzer\Admin\Tree\Filter\Day;
use Brazzer\Admin\Tree\Filter\EndsWith;
use Brazzer\Admin\Tree\Filter\Equal;
use Brazzer\Admin\Tree\Filter\Group;
use Brazzer\Admin\Tree\Filter\Gt;
use Brazzer\Admin\Tree\Filter\Hidden;
use Brazzer\Admin\Tree\Filter\Ilike;
use Brazzer\Admin\Tree\Filter\In;
use Brazzer\Admin\Tree\Filter\Layout\Layout;
use Brazzer\Admin\Tree\Filter\Like;
use Brazzer\Admin\Tree\Filter\Lt;
use Brazzer\Admin\Tree\Filter\Month;
use Brazzer\Admin\Tree\Filter\NotEqual;
use Brazzer\Admin\Tree\Filter\NotIn;
use Brazzer\Admin\Tree\Filter\Scope;
use Brazzer\Admin\Tree\Filter\StartsWith;
use Brazzer\Admin\Tree\Filter\Where;
use Brazzer\Admin\Tree\Filter\Year;
...
class Filter implements Renderable{
    ...
    public function execute($toArray = true){
        $conditions = array_merge($this->conditions(), $this->scopeConditions());
        return $this->model->addConditions($conditions)->buildData($toArray);
    }

    /*public function execute_model($data = true){
        $this->is_run_model = $data;
    }*/
    ...
    public function urlWithoutFilters(){
        ...
        $pageKey = 'page';
        if($this->model->getTree() && $gridName = $this->model->getTree()->getName()){
        ...
        return $this->fullUrlWithoutQuery($columns->merge($groupNames));
    }
